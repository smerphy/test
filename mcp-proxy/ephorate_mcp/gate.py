"""Policy gate: map an MCP tool call to a Ephorate decision.

The protocol-agnostic core of the proxy. ``PolicyGate`` wraps a
:class:`ephorate.EphorateClient` (engine + audit chain + approval flow +
...
* approval   -> resolved inside ``EphorateClient.evaluate``; an unresolved
               require_approval is treated as a deny

Because it drives ``EphorateClient.evaluate``, every gated call is recorded on
...
class GateResult:
    ...
    def error_text(self) -> str:
        ...
        return f"Blocked by Ephorate policy{suffix}: {self.reason}"
