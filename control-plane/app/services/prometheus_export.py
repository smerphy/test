...
Series exposed:
  - ephorate_requests_total{model, status, agent_id}
  - ephorate_tokens_total{model, kind} (input/output/cache_read/cache_write)
  - ephorate_cost_usd_total{model}
  - ephorate_request_duration_ms_summary{model, quantile="0.5|0.95|0.99"}
  - ephorate_alert_rules{state="firing|ok"}
  - ephorate_alert_events_total{severity}

Scrape window is the last 24h to avoid full-table scans; configurable
via `EPHORATE_PROM_WINDOW_HOURS`.
"""
...
def render_prometheus(session: Session, *, org_id: str) -> str:
    hours = int(os.environ.get("EPHORATE_PROM_WINDOW_HOURS", "24"))
    since = _strip_tz(datetime.now(UTC) - timedelta(hours=hours))

    out = StringIO()

    # ephorate_requests_total
    stmt = (
        select(
            MetricEvent.model,
            MetricEvent.status,
    ...
    out.write("# HELP ephorate_requests_total Claude API calls observed.\n")
    out.write("# TYPE ephorate_requests_total counter\n")
    for model, status, agent_id, n in rows:
        labels = _labels({"model": model, "status": status, "agent_id": agent_id})
        out.write(f"ephorate_requests_total{labels} {n}\n")
    ...
    out.write("# HELP ephorate_tokens_total Tokens consumed, summed by model + kind.\n")
    out.write("# TYPE ephorate_tokens_total counter\n")
    out.write("# HELP ephorate_cost_usd_total Dollar spend, summed by model.\n")
    out.write("# TYPE ephorate_cost_usd_total counter\n")
    for model, in_t, out_t, cr_t, cw_t, cost in session.execute(tokens_stmt).all():
        for kind, value in (
            ...
            out.write(
                f"ephorate_tokens_total{_labels({'model': model, 'kind': kind})} {int(value)}\n"
            )
        out.write(
            f"ephorate_cost_usd_total{_labels({'model': model})} {float(cost):.6f}\n"
        )
    ...
    out.write("# HELP ephorate_request_duration_ms_summary Claude call latency (last window).\n")
    out.write("# TYPE ephorate_request_duration_ms_summary summary\n")
    for model, vs in per_model.items():
        for q in (0.5, 0.95, 0.99):
            qlabels = _labels({"model": model, "quantile": str(q)})
            out.write(
                f"ephorate_request_duration_ms_summary{qlabels} "
                f"{_quantile(vs, q):.3f}\n"
            )
        out.write(
            f"ephorate_request_duration_ms_summary_count{_labels({'model': model})} {len(vs)}\n"
        )
        out.write(
            f"ephorate_request_duration_ms_summary_sum{_labels({'model': model})} {sum(vs):.3f}\n"
        )
    ...
    out.write("# HELP ephorate_alert_rules Alert rule count by enabled state.\n")
    out.write("# TYPE ephorate_alert_rules gauge\n")
    for enabled, n in rule_states:
        out.write(
            f"ephorate_alert_rules{_labels({'enabled': str(bool(enabled)).lower()})} {n}\n"
        )
    ...
    out.write("# HELP ephorate_alert_events_total Alert firings in the window, by state.\n")
    out.write("# TYPE ephorate_alert_events_total counter\n")
    for state, n in firings:
        state_value = state.value if hasattr(state, "value") else str(state)
        out.write(
            f"ephorate_alert_events_total{_labels({'state': state_value})} {n}\n"
        )
