...
Prices are per-million-tokens, in USD. The defaults below are
placeholders — set `PRAETOR_CLAUDE_PRICE_BOOK_JSON` to override with
your contract pricing. Whatever the SDK ships as `cost_usd` is
authoritative; this module is the server-side fallback when the SDK
ships only token counts.
"""
...
# Default Claude price book. These are illustrative defaults; override
# via PRAETOR_CLAUDE_PRICE_BOOK_JSON with your contracted rates.
...
def lookup_pricing(model: str) -> ModelPricing | None:
    ...
    if model in book:
        return book[model]
    # Fuzzy: strip date suffix if any (e.g. claude-x-20251001 -> claude-x).
